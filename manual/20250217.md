...
- location : 브라우저 주소차에 관련된 속성 관리
- history : 브라우저 창의 '이전', '다음' 버튼 등 페이지 이동 내역 관리

## 견론 : 브라우저 관련 주요 속성 등이 JS 명세가 아니라 HTML 명세애 정의된 것으로 엄밀치 말하자면 JS의 범위에 들어가있지는 않은데, 정의된  속성을 참조하거나 실행하기 위해서는 JS를 사용해야 하므로 종합적으로 습득해 나가는 것이 FE 개발자로서 나아가는 방향이라고 할 수 있음.
